...
export class BrowserFavicons implements Favicons {
  ...
  constructor(@Inject(BROWSER_FAVICONS_CONFIG) config: FaviconsConfig, @Inject(DOCUMENT) private document: Document, private platformSvc: PlatformService) {
  ...
  private addNode(type: string, href: string): void {
    if (this.platformSvc.isServer || !this.document) {
      return;
    }
  ...
  private removeExternalLinkElements(): void {
    if (this.platformSvc.isServer || !this.document || !document) {
      return;
    }
    var linkElements = this.document.querySelectorAll("link[ rel ~= 'icon' i]");
  ...
  private removeNode(): void {
    ...
    var linkElement = this.document.head.querySelector("#" + this.elementId);
