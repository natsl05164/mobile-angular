...
@Component({
  selector: 'app-direct-deposit-form',
  templateUrl: './direct-deposit-form.container.html',
  styleUrls: ['./direct-deposit-form.container.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DirectDepositFormContainer implements OnInit {
  ...
  onChangeSiteBank() {
    if (this.bank_id.value) {
      let bankInfo: BankInfo = this.accountView.SiteBankList.filter(BankInfo => BankInfo.BankID == this.bank_id.value)[0];
      this.strSiteAccountNo = bankInfo.AccountNo;
      this.strAccHolderName = bankInfo.AccHolderName;
    }
  }
  ...
  onSubmitDeposit() {
    ...
    if (this.depositForm.valid) {
      if (this.bank_id.value) {
        let strAccNo: string =  this.accountView.SiteBankList.filter(BankInfo => BankInfo.BankID == this.bank_id.value)[0].AccountNo
        this.depositForm.controls['SiteAccountNo'].setValue(strAccNo);
      }
  ...
  onSelectAmount(amount:number) {
    this.depositForm.controls["deposit_amount"].setValue(amount);
  }
}
